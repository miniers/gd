...
class Env {
    ...
    async send(username) {
        if (this.msg?.length > 0) {
            const msg = this.msg.concat([`\n时间：${this.now()} 时长：${((this.timestamp() - this.start)
                / 1000).toFixed(2)}s`])
            if (this.bot) {
                await notify.sendNotify("/" + this.name,
                    msg.join(this.delimiter || ''))
            } else {
                if (username) {
                    await notify.sendNotifybyWxPucher(this.name, msg.join("\n"), username)
                } else {
                    await notify.sendNotify(this.name, msg.join("\n"))
    ...
    async sign(fn, body = {}) {
        ...
        try {
            ...
            return { fn: data.functionId, sign: data.url };
        } catch (e) {
